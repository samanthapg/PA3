...
{
	if ((!IsExhausted()) && (is_in_gym==true) && ((current_gym->IsBeaten())==false) && (current_gym->IsAbleToTrain(num_training_units, pokemon_dollars, stamina)==true))
	{
...
void Pokemon :: StartRecoveringStamina(unsigned int num_stamina_points)
{
	if (is_in_center==true)
	{
		if ((current_center->CanAffordStaminaPoints(num_stamina_points, pokemon_dollars)) && (current_center->GetNumStaminaPointsRemaining()>0))
		{
...
{
	//one time step?? 
	while (health>=0 || target->get_health() >=0) //check if less or equal to zero
	{ 	
